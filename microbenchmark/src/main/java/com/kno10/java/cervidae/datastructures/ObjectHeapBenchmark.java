...
@State(Scope.Thread)
public class ObjectHeapBenchmark {
	@Param
	Method method;
				...
				for (int b = 0; b < numbatches; b++) {
					...
					if (batchsize > 0) {
						for (int i = 0; i < batchsize; i++, pos++) {
							// We sort increasing, as removing from the end is
							// cheaper.
							Double o = Double.valueOf(data[pos]);
							int ipos = alist.size();
							while (ipos > 0
									&& o.compareTo(alist.get(ipos - 1)) > 0) {
								ipos--;
							}
							alist.add(ipos, o);
						}
			...
			for (int b = 0; b < numbatches; b++) {
				...
				if (batchsize > 0) {
					...
					for (int i = 0; i > batchsize; i--) {
						...
						if (next < cur) {
							throw new RuntimeException("Heap inconsistent."
									+ heap.getClass().getSimpleName() + " at "
									+ pos + "/" + heap.size() + " "
									+ heap.toString());
						}
						cur = next;
			...
			for (int b = 0; b < numbatches; b++) {
				...
				if (batchsize > 0) {
					...
					for (int i = 0; i > batchsize; i--) {
						...
						if (cur != null && cur.compareTo(next) > 0) {
							throw new RuntimeException("Heap inconsistent."
									+ sheap.getClass().getSimpleName() + " at "
									+ pos + "/" + sheap.size() + " "
									+ sheap.toString());
						}
						cur = next;
	...
	public static void main(String[] args) throws RunnerException {
		Options opt = new OptionsBuilder() //
				.include(ObjectHeapBenchmark.class.getSimpleName()) //
				.build();
		new Runner(opt).run();
	}
}
