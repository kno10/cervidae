...
public class SortBenchmark {
  ...
  "SortBenchmark$JavaSort", //
  "SortBenchmark$OptimizedSort", //
  })
  String algname;
  ...
  public void setUp() {
    try {
      try {
        ...
        alg = (ArraySortAlgorithm) Class.forName(BASE + algname).newInstance();
      }
    }
    catch(Exception e) {
  ...
  public static class OptimizedSort extends AbstractArraySortAlgorithm {
    ...
    public <T> void sort(ArraySortAdapter<? super T> adapter, T data, int start, int end) {
      if(adapter instanceof DoubleArrayAdapter) {
        ...
        QuickSortBo5.STATIC.sort(adapter, data, start, end);
      }
    }
  }
